...
function setSpeed(level) {
    if (level === 'Easy') speed = 3;
    if (level === 'Medium') speed = 5;
    if (level === 'Hard') speed = 9;
    document.getElementById('s').disabled = true;
    document.getElementById('m').disabled = true;
    document.getElementById('f').disabled = true;
    startGame(); // start the game after setting the speed
}
...
function gameEngine() {
    ...
    if(snakeArr[0].y === food.y && snakeArr[0].x === food.x){
        foodSound.play();
        score += speed * 10;
        if(score > hiscoreval.score){
            hiscoreval = { score: score, name: userName };
            localStorage.setItem("hiscore", JSON.stringify(hiscoreval));
            hiscoreBox.innerHTML = `HiScore: ${hiscoreval.score} by ${hiscoreval.name}`;
        }
